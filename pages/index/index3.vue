				    ...
				    <uni-list-item v-for="(item, index) in staffList" :key="index">
						<navigator :url="'/pages/project/stToProTime?st_id='+item.st_id+'&pic_1='+item.pic_1+'&pr_id='+pr_id+'&stname='+item.name">
							...
							<view class="staffDesc">
								...
								<block>状态：
									<block v-if="item.status==0" class="no">不可接单</block>
									<block v-if="item.status==1" class="yes">可接单</block>
									<block  class="distance">{{item.distance}} km</block>
								</block>
								<block>简介：{{item.desc}}</block>
							</view>
						</navigator>
					</uni-list-item>
				</uni-list>
